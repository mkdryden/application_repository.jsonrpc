...
class  TestDumps(unittest.TestCase):
    ...
    def assertJSON(self, json, expectedJSON):
        self.assertTrue(type(json) is UnicodeType)
        self.assertEqual(json, expectedJSON)
           
    def test_Number(self):
        json = jsonrpc.dumps(1)
        self.assertJSON(json, '1')
        
        json = jsonrpc.dumps(0xffffffffffffffffffffffff)
        self.assertJSON(json, '79228162514264337593543950335')

    def test_None(self):
        json = jsonrpc.dumps(None)
        self.assertJSON(json, 'null')
        
    def test_Boolean(self):
        json = jsonrpc.dumps(False)
        self.assertJSON(json, 'false')
        json = jsonrpc.dumps(True)
        self.assertJSON(json, 'true')

    def test_Float(self):
        json = jsonrpc.dumps(1.2345)
        self.assertJSON(json, '1.2345')

        json =jsonrpc.dumps(1.2345e67)
        self.assertJSON(json, '1.2345e+67')
    ...
    def test_StringEscapedUnicodeChars(self):
        json = jsonrpc.dumps('\0 \x19 \x20\u0130')
        self.assertJSON(json, '"\\u0000 \\u0019  \u0130"')

    def test_Array(self):
        json = jsonrpc.dumps([1, 2.3e45, 'foobar'])
        self.assertJSON(json, '[1,2.3e+45,"foobar"]')

    def test_Dictionary(self):
        json = jsonrpc.dumps({'foobar':'spam', 'a':[1,2,3]})
        self.assertJSON(json, '{"a":[1,2,3],"foobar":"spam"}')

    def test_FailOther(self):
        self.assertRaises(jsonrpc.JSONEncodeException, lambda:jsonrpc.dumps(self))
...
class  TestLoads(unittest.TestCase):
    ...
    def test_Dictionary(self):
        ...
        obj = jsonrpc.loads(json)
        self.assertEqual(obj, {'foobar':'spam', 'nested':{'a':'b'}})
    ...
    def test_Float(self):
        ...
        obj = jsonrpc.loads(json)
        self.assertEqual(obj, 1234.567)

    def test_Exponential(self):
        json = jsonrpc.dumps(1234.567e89)
        obj = jsonrpc.loads(json)
        self.assertEqual(obj, 1234.567e89)
    ...
    def test_NestedDictAllTypes(self):
        json = jsonrpc.dumps({'s':'foobar', 'int':1234, 'float':1234.567, 'exp':1234.56e78,
                                            ...
                                            'list':[1,2,4,{}], 'dict':{'a':'b'}})
        obj = jsonrpc.loads(json)
        self.assertEqual(obj, {'s':'foobar', 'int':1234, 'float':1234.567, 'exp':1234.56e78,
                                            'negInt':-1234, 'None':None,'True':True, 'False':False,
                                            'list':[1,2,4,{}], 'dict':{'a':'b'}})
